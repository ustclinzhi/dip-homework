...
def create_mask_from_points(points, img_h, img_w):
    ...
    for y in range(img_h):
        for x in range(img_w):
            if jiaodian(points,x,y) :
                mask[y][x]=255


    return mask

def jiaodian(points,x,y):
    total=0
    n=len(points)
    points2 = np.vstack([points, points[0]])
    if n==0:
        return 0
    for i in range(n):
        if y>min(points2[i][1],points2[i+1][1]) and y<=max(points2[i][1],points2[i+1][1]):
            if points2[i][0]+(y-points2[i][1])*(points2[i+1][0]-points2[i][0])/(points2[i+1][1]-points2[i][1])<x:
                total+=1
    return total%2
...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    ...
    laplacian_foreground = F.conv2d(foreground_img, laplacian_kernel, padding=1, groups=3)
    laplacian_blended = F.conv2d(blended_img, laplacian_kernel, padding=1, groups=3)

    # Compute the masked outputs
    masked_foreground = laplacian_foreground * foreground_mask.unsqueeze(0)
    masked_blended = laplacian_blended * background_mask.unsqueeze(0)

    # 通过裁剪到相同的形状来确保一致性
    if masked_foreground.shape != masked_blended.shape:
        min_height = min(masked_foreground.shape[2], masked_blended.shape[2])
        min_width = min(masked_foreground.shape[3], masked_blended.shape[3])
        
        masked_foreground = masked_foreground[:, :, :min_height, :min_width]
        masked_blended = masked_blended[:, :, :min_height, :min_width]
    ...
    loss = F.mse_loss(masked_foreground, masked_blended)
