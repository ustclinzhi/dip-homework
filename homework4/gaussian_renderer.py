...
class GaussianRenderer(nn.Module):
    ...
    def compute_projection(
        ...
        ### FILL:
        J_proj[:, 0, 0] = 1.0 / depths  # 对 x 的导数
        J_proj[:, 0, 2] = -screen_points[:, 0] / (depths * depths)  # 对 z 的导数 (x 分量)
        J_proj[:, 1, 1] = 1.0 / depths  # 对 y 的导数
        J_proj[:, 1, 2] = -screen_points[:, 1] / (depths * depths)  # 对 z 的导数 (y 分量)
        
        # Transform covariance to camera space
        ### FILL: Aplly world to camera rotation to the 3d covariance matrix
        ### covs_cam = ...  # (N, 3, 3)
        # Transform covariance to camera space
        R_expanded = R.unsqueeze(0).expand(covs3d.size(0), -1, -1)
        print("R shape:", R_expanded.shape)           # Check the shape of R
        print("covs3d shape:", covs3d.shape)  # Check the shape of covs3d
        covs_cam = torch.bmm(R_expanded, torch.bmm(covs3d, R_expanded.transpose(1, 2)))  # (N, 3, 3)

        
        # Project to 2D
        covs2D = torch.bmm(J_proj, torch.bmm(covs_cam, J_proj.transpose(1, 2)))  # (N, 2, 2)
    ...
    def compute_gaussian_values(
        ...
        ### gaussian = ... ## (N, H, W)
        # Compute determinant for normalization
        det_covs = torch.linalg.det(covs2D)  # (N,)
        
        # Compute inverse of covariance matrices
        inv_covs = torch.linalg.inv(covs2D)  # (N, 2, 2)
        
        # Compute Gaussian exponent term
        dx_T = dx.unsqueeze(-2)  # (N, H, W, 1, 2)
        dx_inv_cov_dx = torch.matmul(dx_T, inv_covs.unsqueeze(1).unsqueeze(2))  # (N, H, W, 1, 2) @ (N, 1, 1, 2, 2)
        dx_inv_cov_dx = torch.matmul(dx_inv_cov_dx, dx.unsqueeze(-1))  # (N, H, W, 1, 1)
        exponent = -0.5 * dx_inv_cov_dx.squeeze(-1).squeeze(-1)  # (N, H, W)
        
        # Compute normalization constant
        norm_const = torch.sqrt((2 * torch.pi) ** 2 * det_covs).reshape(N, 1, 1)  # (N, 1, 1)
        
        # Compute Gaussian probability density values
        gaussian = torch.exp(exponent) / norm_const  # (N, H, W)
    ...
    def forward(
        ...
        transmissivity = torch.cumprod(1 - alphas, dim=0)  # (N, H, W)

        # Compute weights
        weights = alphas * transmissivity  # (N, H, W)
