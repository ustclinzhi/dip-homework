...
class GaussianModel(nn.Module):
    ...
    def _init_scales(self, points3D_xyz: torch.Tensor) -> None:
        ...
        dists, _ = knn_points(points, points, K=K)
    ...
    def compute_covariance(self) -> torch.Tensor:
        ...
        # Compute covariance
        ### FILL:
        print("R shape:", R.shape)           # Check the shape of R
        print("S shape:", S.shape)  # Check the shape of covs3d
        S_T = S.transpose(1, 2)  # S.T is (N, 3, 3)
        R_T = R.transpose(1, 2)  # R.T is (N, 3, 3)

        # Compute covariance using batched matrix multiplication
        Covs3d = torch.bmm(R, torch.bmm(S, S_T))  # First part: R @ (S @ S.T)
        Covs3d = torch.bmm(Covs3d, R_T)  # Then multiply by R.T
